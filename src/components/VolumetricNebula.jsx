...
const volumetricFragmentShader = `
  ...
  void main() {
    vec3 pos = vWorldPosition * 0.015;
    pos.x += uTime * 0.01;
    pos.z += uTime * 0.005;
    
    float n1 = fbm(pos);
    float n2 = fbm(pos * 2.0 + vec3(100.0, 50.0, 25.0));
    float n3 = fbm(pos * 0.5 + vec3(50.0, 100.0, 75.0));
    float n4 = fbm(pos * 4.0 + vec3(200.0));
    
    float noise = n1 * 0.4 + n2 * 0.25 + n3 * 0.2 + n4 * 0.15;
    ...
    float falloff = smoothstep(120.0, 30.0, dist);
    
    float heightFalloff = smoothstep(40.0, 0.0, abs(vWorldPosition.y));
    
    float alpha = noise * uDensity * falloff * heightFalloff;
...
export default function VolumetricNebula() {
  ...
  return (
    <mesh ref={meshRef} scale={[150, 80, 150]}>
