...
const PlanetMaterial = shaderMaterial(
  {
    ...
    uFresnelPower: 2.0,
    uNoiseScale: 3.0,
    uAtmosphereColor: new THREE.Color('#00d4ff'),
    uAtmosphereIntensity: 0.5,
  },
  `
    varying vec2 vUv;
  ...
  `
    ...
    void main() {
      ...
      lighting = lighting * 0.7 + 0.3;
      
      vec3 ambient = baseColor * 0.2;
      vec3 diffuse = baseColor * lighting;
      
      float specular = pow(max(dot(reflect(-normalize(vec3(1.0, 1.0, 1.0)), normal), viewDir), 0.0), 32.0);
      specular *= (1.0 - uRoughness) * 0.5;
      
      vec3 emissive = baseColor * uEmissiveIntensity * (1.0 + sin(uTime * 2.0) * 0.2);
      
      vec3 finalColor = ambient + diffuse + specular * vec3(1.0) + emissive;
